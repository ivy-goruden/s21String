...
START_TEST(test_move_string) {
  ...
  char *result = s21_memcpy(dest1, src, n);
  char *expected = memcpy(dest2, src, n);
...
Suite *memcpy_suite(void) {
  Suite *s = suite_create("memcpy_tests");

  TCase *tc_core = tcase_create("Core");
  tcase_add_test(tc_core, test_move_string);
  tcase_add_test(tc_core, test_move_string_8);
